...
Status: audited through the member-invite permission change
Scope: migrations through `20260729221717`, direct PostgREST/RPC clients, and Realtime publication
...
| `public.groups`          | Authenticated `SELECT`, `INSERT`; column-limited identity/unit updates | RLS permits current members to read, the caller to create only a self-owned group, and only the owner to update directly granted columns. Invite permissions use the owner-checked settings RPC. The creation trigger adds the matching owner membership.         |
...
| `public.transactions`    | Authenticated `SELECT`; column-limited `INSERT`                        | RLS permits current group members. Direct inserts can create only IOUs, require the caller, debtor, and creditor to be current members, and force `created_by = auth.uid()`. The settlement kind is not directly insertable. Direct update/delete is not granted. |
...
| `public.get_group_invite_token(uuid)`                                                             | Authenticated RPC               | Returns the private capability only to the owner or a current member while `members_can_invite` is enabled. A permission change takes effect on the next retrieval.                                                          | Current group invite UUID only.                                     |
| `public.rotate_group_invite(uuid)`                                                                | Authenticated RPC               | Requires current ownership, locks the group, rate-limits rotation, replaces the private token, and records token-free activity history.                                                                                      | Replacement group invite UUID.                                      |
| `public.update_group_settings(uuid, text, text, boolean)`                                         | Authenticated RPC               | Requires current ownership and locks the group. Switching to owner-only invitations atomically replaces the member-known token and records token-free rotation history.                                                      | Updated settings plus the owner’s current invite UUID.              |
...
| `public.settle_up(uuid, uuid, numeric)`                                                           | Authenticated RPC               | Rejects a null identity, locks the group, requires both parties to be current members, derives the debtor from `auth.uid()`, and rejects zero, fractional, or excessive settlement quantities against the live pair balance. | One append-only settlement transaction.                             |
...
| `private.serialize_transaction_insert()`                                                          | Transaction insert trigger only | Execution revoked from public API roles; locks the target group row so IOU and settlement inserts share one concurrency boundary.                                                                                            | Trigger row.                                                        |
...
- Member: same-group reads and valid writes; invite retrieval allowed by default and denied after
  the owner disables it; creator spoofing, outsider parties, owner settings, invite-permission
  updates, rotation, and unauthorized reversal denied.
