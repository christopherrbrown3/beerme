...
| Group creation and settings | Insert/update grants plus owner RLS; owner membership trigger; bounded field constraints; owner-only invite-permission updates | A client claims another owner, changes an unrelated group, enables member invitations without ownership, or races ownership/lifecycle actions. Test direct PostgREST paths as every actor.                                                                        | [#38](https://github.com/christopherrbrown3/beerme/issues/38), [#45](https://github.com/christopherrbrown3/beerme/issues/45)                                                                                                                                                                                              |
| Invite join                 | Private revocable UUID capability; permission-checked token retrieval; authenticated `join_group` RPC; pinned `search_path`    | An invite is guessed, leaked, replayed, retrieved by a member after the owner disables sharing, rendered under the wrong account, or auto-accepted. Test invalid-token indistinguishability, explicit confirmation, permission changes, rotation, and throttling. | [#40](https://github.com/christopherrbrown3/beerme/issues/40), [#46](https://github.com/christopherrbrown3/beerme/issues/46), [#70](https://github.com/christopherrbrown3/beerme/issues/70)                                                                                                                               |
...
| Invite UUIDs remain valid bearer capabilities until the owner rotates them.                 | Tokens are generated and stored privately in PostgreSQL, require authentication to redeem, are returned only to the owner or currently authorized members, and QR generation stays on-device. Owners can rotate immediately after suspected disclosure. | Repository owner | 2026-10-29  |
